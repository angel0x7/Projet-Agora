...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['accepter'])) {
        ...
        header("Location: gestion_Vendeur.php");
        exit();
    } elseif (isset($_POST['retirer_statut'])) {
        $userId = $_POST['user_id'];

        // Mettre à jour le type d'utilisateur dans la base de données pour retirer le statut de vendeur
        $stmt = $conn->prepare("UPDATE users SET user_type = 'acheteur' WHERE id = ?");
        $stmt->bind_param("i", $userId);
        $stmt->execute();
        $stmt->close();
