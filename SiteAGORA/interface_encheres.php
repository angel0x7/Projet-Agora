...
<body>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <?php if (isset($_SESSION['user_id']) && $_SESSION['user_id'] != $itemUserId) { ?>
                    ...
                    <?php } else { ?>
                        <div id="temps-ecoule" style="color: red;">Temps écoulé pour les encheres</div>
                        <div id="bidStatus"></div>
                    <?php } ?>
                <?php } else { ?>
                    <div id="temps-restant" style="color: green;"></div>
                    ...
                    <?php
                        if ($highest_bid) {
                            ...
                            echo "<p>Aucune enchère pour le moment.</p>";
                        }
                    ?>
                    ...
                    <?php
                        if ($bids_result->num_rows > 0) {
                            while ($bid = $bids_result->fetch_assoc()) {
                                echo "<div class='media mb-3'>";
                                echo "<img src='" . htmlspecialchars($bid['profile_picture']) . "' class='mr-3' alt='User' width='64'>";
                                echo "<div class='media-body'>";
                                echo "<h5 class='mt-0'>" . htmlspecialchars($bid['name']) . "</h5>";
                                echo "<p>Offre : " . htmlspecialchars($bid['max_bid']) . "€</p>";
                                echo "</div>";
                                echo "</div>";
                            }
                        } else {
    ...
    <script>
        document.getElementById('placeBidButton').addEventListener('click', function () {
            var bidValue = document.getElementById('maxBid').value;
            var productId = <?php echo $itemId; ?>;
            var userId = <?php echo $_SESSION['user_id']; ?>;

            // Créez un objet FormData pour envoyer les données de l'offre
            var formData = new FormData();
            formData.append('product_id', productId);
            formData.append('user_id', userId);
            formData.append('max_bid', bidValue);
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    ...
                    if (response.success) {
                        // Actualisez la page pour montrer la nouvelle enchère
                        bidStatus.innerHTML = '<div class="alert alert-success">Votre enchère a été placée avec succès!</div>';
                        setTimeout(function () {
                            location.reload();
                        }, 1000);
                    } else {
                        bidStatus.innerHTML = '<div class="alert alert-danger">' + response.message + '</div>';
                    }
                }
            };
            xhr.send(formData);
